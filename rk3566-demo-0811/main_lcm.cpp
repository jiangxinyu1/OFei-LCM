 ...
 * @LastEditTime: 2021-09-18 20:13:27
...
{
    // system(" media-ctl -d /dev/media1 --set-v4l2 '\"rkisp-isp-subdev\":0[crop:(0,0)/224x1035]'");
    // system("media-ctl -d /dev/media1 --set-v4l2 '\"rkisp-isp-subdev\":0[fmt:SBGGR12_1X12/224x1035]'");
    // system("media-ctl -d /dev/media1 --set-v4l2 '\"rkisp-isp-subdev\":2[crop:(0,0)/224x1035]'");
    // system("media-ctl -d /dev/media1 --set-v4l2 '\"rkisp-isp-subdev\":2[fmt:SBGGR12_1X12/224x1035]'");
    ...
    if( !node.good() )
    {
        camera_err("LCM is not ready !!! \n");
        return 1;
    }
    imagesHandler handler(&node);
    
    if ( pubImagesRealtime(&camera ,handler) < 0 )
    {
        camera_err("Function: pubImagesRealTime return error\n");
	    return -1;
    }
